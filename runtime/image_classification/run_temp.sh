...
command() {
        ...
        python main_with_runtime.py --module "$MODULE" -b "$1" --data_dir $DATA --rank "$2" --local_rank "$2" --master_addr localhost --config_path "$CONFIG" --distributed_backend gloo --epochs 20 -v 1 -j 10 
...
cd $LOGS_DIR && tar czf ../../logs_temp_"$MODULE".tar *.log && cd - 
