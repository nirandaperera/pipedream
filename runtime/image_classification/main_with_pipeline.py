...
parser.add_argument('-u', '--micro-batch-size', default=256, type=int,
                    metavar='N', help='mini-batch size (default: 256)')
...
def main():
    ...
    # check micro vs batch sizes
    assert args.batch_size % args.micro_batch_size == 0

    # determine shapes of all tensors in passed-in model
    if args.arch == 'inception_v3':
        input_size = [args.micro_batch_size, 3, 299, 299]
    else:
        input_size = [args.micro_batch_size, 3, 224, 224]
    training_tensor_shapes = {"input0": input_size, "target": [args.micro_batch_size]}
    ...
    # todo check
    optimizer = torch.optim.SGD(r.master_parameters,
                                lr=args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=args.micro_batch_size, shuffle=(train_sampler is None),
...
def train(train_loader, r, optimizer, epoch):
    ...
    n = r.num_iterations(loader_size=len(train_loader))  # this is total micro batches
    micro_batches_per_mini_batch = args.batch_size // args.micro_batch_size
